...
RECEIPT_VALUE_COLUMNS = ["VR_RECEITA", "VALOR_RECEITA", "Valor receita", "VR_RECEITA_ESTIMAVEL"]
RECEIPT_TYPE_COLUMNS = ["DS_ORIGEM_RECEITA", "DS_FONTE_RECEITA", "DS_NATUREZA_RECEITA", "Tipo receita", "Fonte do recurso"]
RECEIPT_NATURE_COLUMNS = ["DS_NATUREZA_RECEITA", "DS_ESPECIE_RECEITA", "DS_FONTE_RECEITA", "DS_CNAE_DOADOR"]
...
def file_matches(name: str) -> bool:
    lower = unidecode(Path(name).name).casefold()
    upper = Path(name).name.upper()
    if "_BRASIL." in upper or "_BR." in upper:
        return False
    if "doador_originario" in lower:
        return False
    if "partidos" in lower or "comites" in lower:
        return False
    return "receita" in lower and "candidatos" in lower


def extract_year(year: int, candidates: pd.DataFrame) -> pd.DataFrame:
    ...
    for csv_name, reader in read_zip_csvs(zip_path, chunksize=300_000):
        if not file_matches(csv_name):
            continue
        print(f"{year}: lendo {csv_name}")
        for chunk in reader:
            value_column = find_column(list(chunk.columns), RECEIPT_VALUE_COLUMNS)
            type_column = find_column(list(chunk.columns), RECEIPT_TYPE_COLUMNS)
            nature_column = find_column(list(chunk.columns), RECEIPT_NATURE_COLUMNS)
            candidate_column = find_column(list(chunk.columns), CANDIDATE_ID_COLUMNS)
            if value_column is None or type_column is None or candidate_column is None:
                continue
            chunk[candidate_column] = chunk[candidate_column].astype(str)
            filtered = chunk[chunk[candidate_column].isin(candidate_ids)].copy()
            if filtered.empty:
                continue
            filtered["valor_receita"] = money_to_number(filtered[value_column])
            filtered = filtered[filtered["valor_receita"].gt(0)]
            if filtered.empty:
                continue
            filtered["SQ_CANDIDATO"] = filtered[candidate_column].astype(str)
            filtered["tipo_receita"] = filtered[type_column].map(normalize_label)
            filtered["natureza_receita"] = filtered[nature_column].map(normalize_label) if nature_column else "Sem classificação"
            frames.append(filtered[["SQ_CANDIDATO", "tipo_receita", "natureza_receita", "valor_receita"]])
    if not frames:
        return pd.DataFrame(columns=["ANO_ELEICAO_ANALISE", "SQ_CANDIDATO", "tipo_receita", "natureza_receita", "valor_receita"])
    df = pd.concat(frames, ignore_index=True)
    df["ANO_ELEICAO_ANALISE"] = year
    return df


def build_base(receipts: pd.DataFrame, candidates: pd.DataFrame) -> pd.DataFrame:
    grouped = (
        receipts.groupby(["ANO_ELEICAO_ANALISE", "SQ_CANDIDATO", "tipo_receita", "natureza_receita"], dropna=False)["valor_receita"]
        .sum()
        .reset_index()
    )
    totals = grouped.groupby(["ANO_ELEICAO_ANALISE", "SQ_CANDIDATO"])["valor_receita"].sum().reset_index(name="receita_total_candidato")
    grouped = grouped.merge(totals, on=["ANO_ELEICAO_ANALISE", "SQ_CANDIDATO"], how="left")
    grouped["pct_receita_tipo"] = grouped["valor_receita"] / grouped["receita_total_candidato"].replace(0, np.nan)
    return grouped.merge(candidates, on=["ANO_ELEICAO_ANALISE", "SQ_CANDIDATO"], how="left")


def compare_two_groups(base: pd.DataFrame, candidates: pd.DataFrame, group_column: str, group_a: Any, group_b: Any, label: str) -> pd.DataFrame:
    candidate_totals = base.groupby(["ANO_ELEICAO_ANALISE", "SQ_CANDIDATO"], dropna=False)["receita_total_candidato"].max().reset_index()
    type_totals = base.groupby(["ANO_ELEICAO_ANALISE", "SQ_CANDIDATO", "tipo_receita"], dropna=False)["valor_receita"].sum().reset_index()
    types = base[["ANO_ELEICAO_ANALISE", "ESFERA", "tipo_receita"]].drop_duplicates()
    rows = []
    for year, sphere, receipt_type in types.itertuples(index=False):
        universe = candidates[
        ...
        local = universe.merge(
            type_totals[type_totals["ANO_ELEICAO_ANALISE"].eq(year) & type_totals["tipo_receita"].eq(receipt_type)],
            on=["ANO_ELEICAO_ANALISE", "SQ_CANDIDATO"],
            how="left",
        ).merge(candidate_totals, on=["ANO_ELEICAO_ANALISE", "SQ_CANDIDATO"], how="left")
        local["valor_receita"] = local["valor_receita"].fillna(0.0)
        local["receita_total_candidato"] = local["receita_total_candidato"].fillna(0.0)
        local["pct_receita_tipo"] = (local["valor_receita"] / local["receita_total_candidato"].replace(0, np.nan)).fillna(0.0)
        for variable in ["valor_receita", "pct_receita_tipo"]:
...
def compare_anova(base: pd.DataFrame, candidates: pd.DataFrame) -> pd.DataFrame:
    candidate_totals = base.groupby(["ANO_ELEICAO_ANALISE", "SQ_CANDIDATO"], dropna=False)["receita_total_candidato"].max().reset_index()
    type_totals = base.groupby(["ANO_ELEICAO_ANALISE", "SQ_CANDIDATO", "tipo_receita"], dropna=False)["valor_receita"].sum().reset_index()
    types = base[["ANO_ELEICAO_ANALISE", "ESFERA", "tipo_receita"]].drop_duplicates()
    rows = []
    for year, sphere, receipt_type in types.itertuples(index=False):
        universe = candidates[candidates["ANO_ELEICAO_ANALISE"].eq(year) & candidates["ESFERA"].eq(sphere)][
            ["ANO_ELEICAO_ANALISE", "SQ_CANDIDATO", "IDEOLOGIA_PARTIDO"]
        ].copy()
        universe = universe[universe["IDEOLOGIA_PARTIDO"].isin(["direita", "centro_ou_indefinido", "esquerda"])]
        if universe.empty:
            continue
        local = universe.merge(
            type_totals[type_totals["ANO_ELEICAO_ANALISE"].eq(year) & type_totals["tipo_receita"].eq(receipt_type)],
        ...
        local["pct_receita_tipo"] = (local["valor_receita"] / local["receita_total_candidato"].replace(0, np.nan)).fillna(0.0)
        for variable in ["valor_receita", "pct_receita_tipo"]:
            samples = [part[variable].dropna() for _, part in local.groupby("IDEOLOGIA_PARTIDO") if len(part[variable].dropna()) >= 2]
            row = {"comparacao": "cristaos_direita_centro_esquerda", "ANO_ELEICAO_ANALISE": year, "ESFERA": sphere, "tipo_receita": receipt_type, "variavel": variable, "teste": "anova_um_fator", "n_grupos_validos": len(samples)}
            if len(samples) >= 2:
...
def make_top(df: pd.DataFrame) -> pd.DataFrame:
    out = df[df["variavel"].eq("pct_receita_tipo") & df["p_valor"].lt(0.05) & df["diferenca_media"].notna()].copy()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compara tipos de receita por religião, resultado e ideologia.")
    ...
    receipts = pd.concat([extract_year(year, candidates) for year in args.years], ignore_index=True)
    base = build_base(receipts, candidates)

    bases_dir = output_dir / "bases"
    bases_dir.mkdir(parents=True, exist_ok=True)
    write_csv(base, bases_dir / "base_receitas_tipos_comparativa.csv")
    base.to_parquet(PROCESSED_DIR / "receitas_tipos_comparativa.parquet", index=False)
    ...
    elected_tests = compare_two_groups(christian_base, christian_candidates, "ELEITO", True, False, "cristaos_eleitos_vs_derrotados")
    ...
    ideology_anova = compare_anova(christian_base, christian_candidates)

    write_csv(elected_tests, output_dir / "testes_receitas_tipos_cristaos_eleitos_vs_derrotados.csv")
    write_csv(religion_tests, output_dir / "testes_receitas_tipos_cristaos_vs_nao_cristaos.csv")
    write_csv(ideology_tests, output_dir / "testes_receitas_tipos_cristaos_ideologia_pares.csv")
    write_csv(ideology_anova, output_dir / "anova_receitas_tipos_cristaos_ideologia.csv")
    write_csv(make_top(pd.concat([elected_tests, religion_tests, ideology_tests], ignore_index=True)), output_dir / "top_diferenciais_receitas_tipos_comparativos.csv")
    print("Análise comparativa de tipos de receita concluída.")
